...
DHANDLE CImageObject::GetMetafilePict (
	void
) {
		// Allocate the structure
	DHANDLE			hMFP;
	hMFP = (DHANDLE)::GlobalAlloc (GMEM_SHARE | GHND, sizeof (METAFILEPICT));
	if (((DHANDLE) NULL) == hMFP)
		return (hMFP);

		// Lock
	LPMETAFILEPICT	lpMFP = (LPMETAFILEPICT) ::GlobalLock ((HGLOBAL)hMFP);
