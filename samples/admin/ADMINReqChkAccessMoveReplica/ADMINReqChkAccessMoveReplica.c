...
PROGRAM:    ADMINReqChkAccessMoveReplica

FILE:       ADMINReqChkAccessMoveReplica.c

PURPOSE:    This program will  creates a "Check Access for Move Replica Creation" request in the Administration Requests database (admin4.nsf).

SYNTAX:     ADMINReqChkAccessMoveReplica  <source server name> <source database name> <destination server name> <destination full path database name>
...
int main(int argc, char *argv[])
{
    ...
    if (error = ADMINReqChkAccessMoveReplica(db_handle,
                                            chAuthorName,
                                            pchSrcServer,
                                            srcDBFullPath,
                                            chTitle,
                                            ...
                                            sizeof(arpPtr)))
    {
        PRINTERROR(error, "ADMINReqChkAccessMoveReplica");
        NSFDbClose (srcDB_handle);
        NSFDbClose(db_handle);
        NotesTerm();
        return (1);
    ...
    PRINTLOG("\n!!! ADMINReqChkAccessMoveReplica Processed Sucessfully !!! \n ");
