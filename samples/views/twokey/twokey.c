...
/* Processing input arguments */
   memset(dbFilename, '\0', DBNAME_LEN);
   memset(ViewName, '\0', VIEWNAME_LEN);
   memset(Key1, '\0', STRING_LENGTH);
   memset(Key2, '\0', STRING_LENGTH);
                               ...
                               char *dbFileName, char *ViewName)
{
    if (argc != 5)
    {
        ...
        fflush(stdout);
        fgets(dbFileName, DBNAME_LEN-1, stdin);

        printf("\n");
        printf("Enter the view Name: ");
        fflush(stdout);
        fgets(ViewName, VIEWNAME_LEN-1, stdin);
 
    }
    else
    {
         strncpy(dbFileName, argv[1], DBNAME_LEN-1);
         strncpy(ViewName, argv[2], VIEWNAME_LEN-1);
